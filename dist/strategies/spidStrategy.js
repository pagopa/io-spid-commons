 ...
 */
function loadFromRemote(idpMetadataUrl, idpIds) {
    return TaskEither_1.tryCatch(() => {
        logger_1.log.info("Fetching SPID metadata from [%s]...", idpMetadataUrl);
        return idpLoader_1.fetchIdpMetadata(idpMetadataUrl);
    }, error => error)
        .map(idpMetadataXML => {
        logger_1.log.info("Parsing SPID metadata...");
        return idpLoader_1.parseIdpMetadata(idpMetadataXML);
    })
        .chain(TaskEither_1.fromPredicate(idpMetadata => idpMetadata.length > 0, () => {
...
exports.loadSpidStrategy = (config) => {
    const idpOptionsTasks = [
        loadFromRemote(config.IDPMetadataUrl, exports.IDP_IDS)
    ].concat(config.hasSpidValidatorEnabled
        ? [
    ...
    return Array_1.array
        .sequence(TaskEither_1.taskEither)(idpOptionsTasks)
        .map(idpOptionsRecords => idpOptionsRecords.reduce((prev, current) => (Object.assign({}, prev, current)), {}))
        .map(idpOptionsRecord => {
        logSamlCertExpiration(config.samlCert);
        const options = {
            idp: Object.assign({}, idpOptionsRecord, { xx_servizicie_test: {
                    cert: [
